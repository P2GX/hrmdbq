...
async fn fetch_citation(numerical_pmid: &str) -> Result<Citation, String> {
    ...
    let formatted_authors = article_record
        .authors
        .iter()
    ...
    let title = match &article_record.title {
        Some(t) => t.to_string(),
        None => {
            return Err("Could not retrieve title from record".to_string());
        }
    };
    ...
    let journal = match &article_record.journal {
        Some(j) => j.to_string(),
        None => {
            return Err("Could not retrieve journal from record".to_string());
        }
    };
    ...
    let volume = match &article_record.volume {
        Some(v) => v.to_string(),
        None => {
            return Err("Could not retrieve volume from record".to_string());
        }
    };
    ...
    let pages = match &article_record.pages {
        Some(p) => p.to_string(),
        None => {
            return Err("Could not retrieve pages from record".to_string());
        }
    };
